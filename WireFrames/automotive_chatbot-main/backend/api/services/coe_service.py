...
class COEService:
    ...
    def format_coe_response(self, coe_data: COEData) -> str:
        """
        Format COE data for chatbot response
        """
        response = f"🚗 **Latest COE Bidding Results ({coe_data.bidding_date})**\n\n"
        
        for result in coe_data.results:
            response += f"**{result.category}**\n"
            response += f"💰 Premium: ${result.quota_premium:,.2f}\n"
            response += f"📊 Bids: {result.bids_successful}/{result.bids_received} successful\n\n"
        
        response += f"*Source: {coe_data.source}*\n"
        response += "Need help with vehicle financing or registration? I can assist! 🚙"
