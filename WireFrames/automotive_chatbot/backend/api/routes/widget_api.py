...
from api.services.rag_service import AutomotiveRAGService

router = APIRouter()

# Initialize services
coe_service = COEService()
rag_service = AutomotiveRAGService()
...
async def widget_chat(message_data: WidgetMessage):
    """
    Main chat endpoint for widget integration
    Supports both basic and RAG-enhanced responses
    """
    try:
        ...
        # Add Singapore context by default for automotive queries
        enhanced_context = {
            "location": "singapore",
            "domain": "automotive",
            **context
        ...
        rag_response = await rag_service.get_intelligent_response(
            message, 
            enhanced_context
        )
        
        # Use RAG response if available and confident
        if rag_response["confidence"] > 0.6:
            response_text = rag_response["response"]
            source = "ai_enhanced"
        else:
            # Fallback to rule-based responses
            response_text = await process_widget_message(message, enhanced_context)
            source = "rule_based"
        ...
        return {
            ...
            "source": source,
            "metadata": {
                "timestamp": rag_response.get("timestamp") if rag_response["confidence"] > 0.6 else None,
